  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(`/api/admin/clinical?type=lab_order`, {
        ...
        body: JSON.stringify({
          patientName,
          toothNumbers: toothNumbers.split(",").map((n) => Number(n.trim())).filter(Boolean),
          prostheticType,
          shade,
          labPartner,
          status: "IMPRESSION_TAKEN",
          impressionDate: new Date().toISOString().split("T")[0],
          expectedDate: expectedDate || new Date(Date.now() + 5 * 86400000).toISOString().split("T")[0],
    ...
    <div className="space-y-4">
      ...
      <div className="bg-slate-900/90 border border-slate-800 p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-4 py-2 bg-purple-500 hover:bg-purple-400 text-slate-950 font-bold text-xs rounded-xl shadow-lg transition flex items-center gap-1.5"
          >
      ...
      <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2 flex-1">
          ...
          <input
            ...
            className="bg-transparent border-none text-white focus:outline-none w-full placeholder-slate-500 text-xs sm:text-sm"
          />
        </div>
        ...
        <select
          ...
          className="bg-slate-950 border border-slate-800 text-slate-300 rounded-xl px-3 py-1.5 focus:outline-none"
        >
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-800 text-slate-400 bg-slate-950/50">
                ...
                <th className="py-3 px-3.5">Patient &amp; Tooth</th>
                <th className="py-3 px-3">Prosthetic Type &amp; Shade</th>
                <th className="py-3 px-3">Lab Partner</th>
                <th className="py-3 px-3">Expected Date</th>
                <th className="py-3 px-3">Fee / Margin</th>
                <th className="py-3 px-3">Status</th>
                <th className="py-3 px-3.5 text-right">Advance Stage</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/60">
              {filteredOrders.map((o) => (
                <tr key={o.id} className="hover:bg-slate-800/40 transition">
                  <td className="py-3.5 px-3.5 font-mono font-bold text-amber-300">
                    {o.orderNumber}
                  </td>

                  <td className="py-3.5 px-3.5">
                    <span className="font-bold text-white block">{o.patientName}</span>
                  ...
                  <td className="py-3.5 px-3">
                    <span className="font-semibold text-slate-200 block truncate max-w-xs">{o.prostheticType}</span>
                  ...
                  <td className="py-3.5 px-3 text-slate-300">
                    <span className="font-medium block">{o.labPartner}</span>
                    <span className="text-[10px] text-slate-500">Sent: {o.impressionDate}</span>
                  </td>

                  <td className="py-3.5 px-3">
                    <span className="font-mono text-white flex items-center gap-1">
                      <Calendar className="w-3 h-3 text-purple-400" /> {o.expectedDate}
                    </span>
                  ...
                  <td className="py-3.5 px-3 font-mono">
                    <span className="text-white font-bold block">₹{o.patientCharge.toLocaleString("en-IN")}</span>
                    <span className="text-[10px] text-slate-400">Lab Cost: ₹{o.costToClinic}</span>
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-1.5">
                      {o.status === "IMPRESSION_TAKEN" && (
                        <button
                          ...
                          className="py-1 px-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-[11px] font-semibold transition"
                        >
                          Dispatch to Lab ➔
                        </button>
                      )}
                      {o.status === "SENT_TO_LAB" && (
                        <button
                          ...
                          className="py-1 px-2 rounded-lg bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 border border-purple-500/30 text-[11px] font-semibold transition"
                        >
                          In Fabrication ➔
                        </button>
                      )}
                      {o.status === "IN_FABRICATION" && (
                        <button
                          ...
                          className="py-1 px-2 rounded-lg bg-sky-600/20 hover:bg-sky-600/30 text-sky-300 border border-sky-500/30 text-[11px] font-semibold transition"
                        >
                          Trial Received ➔
                        </button>
                      )}
                      {o.status === "TRIAL_RECEIVED" && (
                        <button
                          ...
                          className="py-1 px-2 rounded-lg bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 border border-emerald-500/30 text-[11px] font-semibold transition"
                        >
                          Ready to Fit ➔
                        </button>
                      )}
                      {o.status === "READY_FOR_CEMENTATION" && (
                        <button
                          ...
                          className="py-1 px-2 rounded-lg bg-emerald-500 text-slate-950 font-bold text-[11px] transition shadow"
                        >
                          Mark Cemented ✓
                        </button>
                      )}
      ...
      {isNewModalOpen && (
        ...
        >
          <div
            data-lenis-prevent
            className="bg-slate-900 border border-amber-500/30 rounded-3xl w-full max-w-lg p-6 shadow-2xl space-y-4 overscroll-contain max-h-[90vh] overflow-y-auto"
          >
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h4 className="text-base font-bold text-white">Send New Dental Lab Order</h4>
            ...
            <form onSubmit={handleCreateOrder} className="space-y-3.5 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white focus:outline-none focus:border-amber-500 text-xs sm:text-sm"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Tooth FDI Numbers *</label>
                  <input
                    type="text"
                    required
                    value={toothNumbers}
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">VITA Shade *</label>
                  <select
                    value={shade}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Prosthetic Type</label>
                  <select
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Lab Partner</label>
                  <select
                    value={labPartner}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Patient Charge (₹)</label>
                  <input
                    type="number"
                    value={patientCharge}
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Special Lab Instructions / Occlusion</label>
                <input
                  type="text"
                  value={notes}
              ...
              <div className="pt-3 flex gap-2">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 py-2.5 rounded-xl bg-purple-500 hover:bg-purple-400 text-slate-950 font-bold transition"
                >
