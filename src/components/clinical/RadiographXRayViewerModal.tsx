...
export default function RadiographXRayViewerModal({
  ...
  return (
    ...
    >
      <div
        data-lenis-prevent
        className="bg-slate-900 border border-amber-500/30 rounded-3xl w-full max-w-5xl p-4 sm:p-6 shadow-2xl space-y-4 overscroll-contain max-h-[95vh] overflow-y-auto flex flex-col justify-between"
      >
        {/* Modal Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full bg-sky-500/10 text-sky-300 border border-sky-500/30 text-[10px] font-bold">
                  {selectedRad?.type || "IOPA"}
                </span>
              </div>
              <p className="text-xs text-slate-400">
                Patient: <strong className="text-white">{patient?.name || selectedRad?.patientName || "Aarav Sharma"}</strong> • Tooth:{" "}
                <span className="text-amber-400 font-mono font-bold">#{selectedRad?.toothNumber || "16"}</span>
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1">
          ...
          <div className="lg:col-span-2 bg-black rounded-2xl border border-slate-800 relative overflow-hidden flex flex-col items-center justify-center min-h-[380px] p-4 select-none">
          ...
          <div className="space-y-3.5 bg-slate-950/80 p-4 rounded-2xl border border-slate-800 flex flex-col justify-between text-xs">
            <div className="space-y-3">
              <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                <span className="font-bold text-slate-300 flex items-center gap-1.5">
                  <Sliders className="w-3.5 h-3.5 text-amber-400" /> Digital RVG Filters
                </span>
                <button
              ...
              <button
                ...
                className={`w-full py-2 px-3 rounded-xl border font-bold transition flex items-center justify-between ${
                  inverted
              ...
              >
                ...
                <span>{inverted ? "ON (Dark Inverted)" : "OFF"}</span>
              </button>
              ...
              <div className="space-y-1 pt-2 border-t border-slate-800">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-900 border border-slate-800 rounded-xl p-2 text-white font-mono focus:outline-none focus:border-amber-500"
                  />
              ...
              <div className="space-y-1">
                <label className="block text-slate-300 font-semibold">Radiographic Findings</label>
                <textarea
                  rows={2}
                  value={findings}
                  onChange={(e) => setFindings(e.target.value)}
                  placeholder="e.g. Periapical radiolucency on root apex"
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2 text-slate-200 focus:outline-none focus:border-amber-500 text-xs"
                />
              </div>
            </div>
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-xl shadow-lg transition flex items-center justify-center gap-1.5"
              >
