...
export default function ConsentFormModal({
  ...
  return (
    ...
    >
      <div
        data-lenis-prevent
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-amber-500/30 rounded-3xl w-full max-w-3xl p-4 sm:p-6 shadow-2xl space-y-4 overscroll-contain max-h-[92vh] overflow-y-auto flex flex-col justify-between text-[#1C1A17] dark:text-white"
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#7A7265] dark:text-slate-400">
                Patient: <strong className="text-[#1C1A17] dark:text-white font-bold">{patient?.name || "Aarav Sharma"}</strong> • Dr. Shreya Nidhi
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs text-amber-800 dark:text-amber-300 font-bold flex items-center gap-1.5 hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer"
            >
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-[#7A7265] dark:text-slate-400 hover:text-black dark:hover:text-white cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition whitespace-nowrap cursor-pointer ${
                treatmentType === tab.id
                  ...
                  : "bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-400 hover:bg-slate-200 dark:hover:text-white"
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="bg-slate-50 dark:bg-slate-950 p-4 sm:p-5 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs space-y-4 text-slate-800 dark:text-slate-300">
          <div className="border-b border-slate-200 dark:border-slate-800 pb-2 flex justify-between items-center text-[#7A7265] dark:text-slate-400 text-[11px] font-medium">
            <span>Amulyam Dental Studio, Awadhpuri Bhopal</span>
            <span>Date: {new Date().toLocaleDateString("en-IN")}</span>
          </div>

          <h4 className="text-sm font-extrabold text-[#1C1A17] dark:text-white">{currentTemplate.title}</h4>
          ...
          <div className="space-y-2 bg-white dark:bg-slate-900/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800/80">
            ...
            {currentTemplate.risks.map((risk, idx) => (
              <label key={idx} className="flex items-start gap-2 cursor-pointer text-slate-700 dark:text-slate-300">
                <input
                  type="checkbox"
          ...
          <div className="space-y-1.5 pt-2">
            <div className="flex justify-between items-center text-xs">
              <span className="font-bold text-[#1C1A17] dark:text-white flex items-center gap-1.5">
                <PenTool className="w-3.5 h-3.5 text-amber-500" /> Patient Digital Signature (Sign with finger / mouse) *
              </span>
              <button
                type="button"
                onClick={clearSignature}
                className="text-[11px] text-amber-700 dark:text-amber-400 hover:underline flex items-center gap-1 font-bold cursor-pointer"
              >
                <RotateCcw className="w-3 h-3" /> Clear Signature
              </button>
            </div>

            <div className="border-2 border-dashed border-amber-500/40 rounded-xl bg-white dark:bg-slate-900 overflow-hidden relative shadow-inner">
              <canvas
                ref={canvasRef}
                width={650}
                height={120}
          ...
          <div className="pt-2 flex justify-between items-center text-[11px] text-[#7A7265] dark:text-slate-400 border-t border-slate-200 dark:border-slate-800">
            <div>
              <span className="font-bold text-[#1C1A17] dark:text-white block">Dr. Shreya Nidhi (BDS, MDS)</span>
        ...
        <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-200 dark:border-slate-800 text-xs">
          ...
          <button
            type="button"
            onClick={handlePrint}
            className="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-white rounded-xl font-bold flex items-center gap-1.5 cursor-pointer"
          >
          ...
          <button
            ...
            className="px-5 py-2.5 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-slate-950 font-bold rounded-xl shadow-md transition flex items-center gap-1.5 cursor-pointer"
          >
